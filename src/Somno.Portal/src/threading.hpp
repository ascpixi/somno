...
bool somno_create_thread(LPTHREAD_START_ROUTINE target, PVOID parameter) {
	...
	DWORD oldProtect;
	VirtualProtect((LPVOID)0x40000, 0x1000, PAGE_EXECUTE_READWRITE, &oldProtect);
	...
	ctxSuccess = GetThreadContext(hThread, &ctx);
