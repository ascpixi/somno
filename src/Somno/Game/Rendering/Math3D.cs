...
internal static class Math3D
{
    ...
    /// <summary>
    /// Calculates the angle between 3D vectors.
    /// </summary>
    public static float AngleTo(this Vector3 vector, Vector3 other)
        => AngleBetweenUnitVectors(vector.Normalize(), other.Normalize());

    /// <summary>
    /// Calculates the angle between 3D unit vectors (normalized vectors).
    /// </summary>
    public static float AngleBetweenUnitVectors(Vector3 leftNormalized, Vector3 rightNormalized)
            => AcosClamped(leftNormalized.Dot(rightNormalized));
    ...
    public static void GetOrthogonalAxis(Vector3 normal, out Vector3 xAxis, out Vector3 yAxis, out Vector3 zAxis)
    {
        ...
        if (angleToAxisZ is < (float)(Math.PI * 0.25) or > (float)(Math.PI * 0.75)) {
